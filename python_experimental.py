...
moon_begin_speed = [0, 10180]

cooldown = 100

class Planet:
    def __init__(self, position, speed, radius, mass, color, immobile=False):
...
planets = [Planet([0, 0], [0, 0], 6371000, 5.972e24, (0, 0, 255), immobile=True),
...
def update(dt):
    global sim_space_multiplier, sim_radius_multiplier, key_cooldown, cooldown, moon_begin_speed, planets
    key_cooldown -= 1

    for planet in planets:
        if planet.position[0] < 0 or planet.position[0] > WIDTH / sim_space_multiplier or planet.position[1] < 0 or planet.position[1] > HEIGHT / sim_space_multiplier:
            print(f"Planet out of bounds at position: ({planet.position[0]}, {planet.position[1]})")
            planets.remove(planet)

    if planets.__len__() < 2:
        #moon_begin_speed = [0, moon_begin_speed[1]]
        planets.append(Planet([384400000, 0], moon_begin_speed, 1737000, 7.34767309e22, (255, 255, 255)))
